...
export const renderProjects = function (container, projectStore, filter = "all") {
  ...
  filteredProjects.forEach((project) => {
    ...
    if (safeProject.video) {
      ...
      // Use <picture> so the browser can pick the best source for the viewport.
      // On narrow screens (max-width: 600px) the same image is used but the browser
      // already constrains it via CSS, keeping layout clean without needing extra files.
      ...
      // Source hint for small screens - same file, but the rule keeps the pattern clear.
      const sourceSmall = document.createElement("source");
      sourceSmall.media = "(max-width: 600px)";
      sourceSmall.srcset = safeProject.image;

      // Default fallback <img> used by all browsers.
      ...
      // Use a default image if the original path is broken.
